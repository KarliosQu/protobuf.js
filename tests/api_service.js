...
tape.test("reflected services", function(test) {
    ...
    test.same(JSON.parse(JSON.stringify(MyService.toJSON())), def, "should construct without methods from and convert back to JSON");
    ...
    test.same(JSON.parse(JSON.stringify(MyService.toJSON())), def, "should convert to initial JSON afterwards");

    def.methods.MyMethod = methodDef;
    MyService = protobuf.Service.fromJSON("MyService", def);
    test.same(JSON.parse(JSON.stringify(MyService.toJSON())), def, "should construct with methods from and convert back to JSON");
