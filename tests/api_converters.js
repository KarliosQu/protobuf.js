    ...
    protobuf.load("tests/data/convert.proto", function(err, root) {
        ...
        test.test(test.name + " - Message#toObject", function(test) {
            ...
            test.test(test.name + " - called with defaults = true", function(test) {
                ...
                test.same(obj.uint64Val && obj.uint64Val.low !== undefined ? { low: obj.uint64Val.low, high: obj.uint64Val.high, unsigned: obj.uint64Val.unsigned } : obj.uint64Val, { low: 0, high: 0, unsigned: true }, "should set uint64Val");
        ...
        test.test(test.name + " - Message.fromObject", function(test) {
            ...
            test.same(msg.uint64Val && msg.uint64Val.low !== undefined ? { low: msg.uint64Val.low, high: msg.uint64Val.high, unsigned: msg.uint64Val.unsigned } : msg.uint64Val, { low: 1, high: 0, unsigned: true }, "should set uint64Val from a number");
            test.same(msg.uint64Repeated.map(function(l) { return l && l.low !== undefined ? { low: l.low, high: l.high, unsigned: l.unsigned } : l; }), [ { low: 1, high: 0, unsigned: true}, { low: 2, high: 0, unsigned: true } ], "should set uint64Repeated from a number and a string");
            test.same(msg.bytesVal, buf, "should set bytesVal from a base64 string");
            test.same(msg.bytesRepeated.map(function(b) { return Buffer.from(b); }), [ buf, buf ], "should set bytesRepeated from a base64 string and a plain array");
            ...
            var mapPlain = {}; Object.keys(msg.int64Map).forEach(function(k) { var l = msg.int64Map[k]; mapPlain[k] = l && l.low !== undefined ? { low: l.low, high: l.high, unsigned: l.unsigned } : l; });
            test.same(mapPlain, { a: { low: 2, high: 0, unsigned: false }, b: { low: 3, high: 0, unsigned: false } }, "should set int64Map from a number and a string");
